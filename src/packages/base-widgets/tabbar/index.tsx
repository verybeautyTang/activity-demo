...
export default {
  ...
  render: ({ props, block }) => {
    ...
    onMounted(() => {
      ...
      const tabbarEl = draggableEl?.querySelector(
        ".van-tabbar"
      ) as HTMLDivElement;
    ...
    return () => (
      ...
      >
        {props.tabs?.map(item => {
          ...
          const url = `${BASE_URL}${props.baseUrl}${itemProps.url}`.replace(
            /\/{2,}/g,
            "/"
          );
          return (
    ...
    tabs: createEditorCrossSortableProp({
      ...
      defaultValue: defaultTabbarItems.map(item => {
        ...
        return {
          label: item.title,
          value: item.icon,
          component: getTabbarItem(),
          block,
        };
      }),
    }),
    fixed: createEditorSwitchProp({
      label: "是否固定在底部",
      defaultValue: true,
    }),
    border: createEditorSwitchProp({
      label: "是否显示外边框",
      defaultValue: true,
    }),
    zIndex: createEditorInputProp({ label: "元素 z-index", defaultValue: "1" }),
    baseUrl: createEditorInputProp({
      label: "路由路径前缀",
