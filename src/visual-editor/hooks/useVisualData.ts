...
// 注入jsonData的key
export const injectKey: InjectionKey<ReturnType<typeof initVisualData>> =
  Symbol();
...
export const initVisualData = () => {
  ...
  const jsonData: VisualEditorModelValue = Object.keys(localData?.pages || {})
    .length
    ? localData
    : defaultValue;
  ...
  const state: IState = reactive({
    ...
    currentPage,
    currentBlock:
      currentPage?.blocks?.find(item => item.focus) ??
   ...
   */
  const updateFetchApi = (
    api: FetchApiItem | FetchApiItem[],
    isCover = false
  ) => {
   ...
   */
  const updateModel = (
    model: VisualEditorModel | VisualEditorModel[],
    isCover = false
  ) => {
