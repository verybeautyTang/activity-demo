<template>
  <div class="!mb-10px">
    ...
    <el-button type="warning" @click="showImportSwaggerJsonModal"
      >导入swagger</el-button
    >
    <el-popconfirm
      title="确定要删除全部接口吗？"
...
const { jsonData, incrementFetchApi, updateFetchApi, deleteFetchApi } =
  useVisualData();
...
const isEdit = computed(() =>
  apis.value.some(item => item.key == state.ruleForm.key)
);
    ...
    content: () => (
      ...
      >
        ...
        <ElFormItem label="接口" prop={"options.url"}>
          <ElInput
            v-model={state.ruleForm.options.url}
            placeholder={"请输入接口地址"}
          >
            {{
              prepend: () => (
                <ElSelect
                  v-model={state.ruleForm.options.method}
                  class={"w-90px"}
                >
